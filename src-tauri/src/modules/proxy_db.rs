...
pub fn init_db() -> Result<(), String> {
    ...
    let _ = conn.execute("ALTER TABLE request_logs ADD COLUMN upstream_url TEXT", []);
    let _ = conn.execute(
    ...
    let logs_iter = stmt
        .query_map([limit, offset], |row| {
            Ok(ProxyRequestLog {
                ...
                upstream_protocol: row.get(18).unwrap_or(None),
                upstream_model: row.get(19).unwrap_or(None),

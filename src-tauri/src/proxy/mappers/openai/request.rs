...
pub fn to_claude_body(request: &OpenAIRequest) -> Value {
    ...
    if let Some(t) = request.temperature {
        body["temperature"] = serde_json::Number::from_f64(t).unwrap_or(serde_json::Number::from_f64(1.0).unwrap()).into();
    }
    if let Some(t) = request.top_p {
        body["top_p"] = serde_json::Number::from_f64(t).unwrap_or(serde_json::Number::from_f64(1.0).unwrap()).into();
    }

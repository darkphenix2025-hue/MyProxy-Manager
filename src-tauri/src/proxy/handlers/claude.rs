...
pub async fn handle_messages(
    ...
    let translator_enabled = state
        ...
        .read()
        .await
        .is_format_enabled("claude");
    if translator_enabled {
        tracing::debug!("[Translator] New translator path enabled for Claude requests");
    }
    ...
    // Update request model to stripped/resolved version if provider_id routing was used
    if let Some(ref resolved) = resolved_model {
        request.model = resolved.clone();
    }
    ...
    // Route to upstream provider or legacy z.ai path
    if use_provider {
        ...
        match route_protocol {
            ...
            Some("openai_compat") => {
                if let Some(provider) = provider {
                    // 当 translator 启用时，使用 translator 框架做 Claude→OpenAI 转换
                    if translator_enabled {
                        return crate::proxy::providers::zai_openai_compat::forward_claude_via_openai_compat_with_translator(
            ...
            Some("gemini_v1") => {
                // GeminiV1Internal for Claude input: fall through to Google Flow
                tracing::debug!("[{}] GeminiV1Internal selected for Claude request, falling through to Google path", trace_id);
            }
            _ => {}
        }
        ...
        let new_body = match serde_json::to_value(&request) {
            Ok(v) => v,
            Err(e) => {
                tracing::error!("Failed to serialize fixed request for legacy z.ai: {}", e);
                return StatusCode::INTERNAL_SERVER_ERROR.into_response();
            }
        };
